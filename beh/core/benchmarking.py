...
def get_optimal_batch_size(
    ...
    '''
    speed = []
    start_batch_size, optimal_batchsize = 4096, 0
    max = 15
    ...
    for i in range(1, max):
        try:
            ...
            jax.clear_caches()
            gc.collect()
        print(f'{i}/{max} tested batch size {int(start_batch_size * 2**i)} on {int(query_size / 1e06)}M queries -> {func.__name__}() | Inf. Speed: {speed[-1]}ms')
    ...
    plt.title(f"Batch Size - Inference Speed relation for {model_key} -> Best {optimal_batchsize}")
...
def benchmark_inference_speed(
    ...
    '''
    Measures the inference speed of a model given its forward call for N queries, averaged over M iterations.
    '''
    @jax.jit
    def batch_INF(i, val):
        start = i * batch_size
