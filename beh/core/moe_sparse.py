...
@jax.jit
def moe_forward_sparse_2048(p : dict, x : jax.Array):
    logits = moe_forward_gate(p['gate'], x)
    x, unpack_mini = minibatch_2048(x, logits)
    out = jax.vmap(lambda p, x: moe_forward_expert(p, x))(p['experts'], x)
    return out[unpack_mini[...,0], unpack_mini[...,1]]

@jax.jit
def moe_forward_sparse_INF_2048(p : dict, x : jax.Array):
    return jax.nn.sigmoid(moe_forward_sparse_2048(p, x))

@jax.jit
def minibatch_2048(x, logits):
    # Constants from config
    nex = configs['moe']['nex']
    in_dim = args.dim
    minibatch_size = 2048
